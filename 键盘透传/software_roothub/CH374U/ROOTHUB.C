/* CH374芯片 应用层 V1.0 */
/* USB主机,有关Root-HUB的应用例子,初始化和枚举当前根集线器连接的USB设备,同时支持最多3个USB设备,支持一级外部HUB */
...
struct _RootHubDev RootHubDev[3];

struct _DevOnHubPort DevOnHubPort[3][4];
UINT8	NewDevCount;
UINT8	CtrlBuf[8];
UINT8	TempBuf[64];
...
// 当然也可以根据USB的厂商VID产品PID进行搜索(事先要记录各设备的VID和PID),以及指定搜索序号
	...
	if ( i != 0xFF ) return( (UINT16)i << 8 );  // 在ROOT-HUB端口上
	for ( i = 0; i < 3; i ++ ) {  // 现时搜索可以避免设备中途拔出而某些信息未及时更新的问题
