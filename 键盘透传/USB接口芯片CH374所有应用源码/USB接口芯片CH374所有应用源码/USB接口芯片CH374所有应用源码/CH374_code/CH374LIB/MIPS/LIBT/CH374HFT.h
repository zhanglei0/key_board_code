...
**  TC2.0@PC, cygwin gcc 3.0.4@MIPS   **
****************************************
*/
/* CH374 主机文件系统接口 */
...
#define	BIT_HUB_PRE_PID		0x40		/* 低速前置包PRE PID输出控制: 0-允许(外部设备是USB-HUB), 1-禁止 */
...
UINT8  DISK_BASE_BUF[ DISK_BASE_BUF_LEN ];	/* 外部RAM的磁盘数据缓冲区,缓冲区长度为一个扇区的长度,起始地址必须为8字节边界地址 */
#else
UINT8  DISK_BASE_BUF[ DISK_BASE_BUF_LEN ];	/* 外部RAM的磁盘数据缓冲区,缓冲区长度为一个扇区的长度,起始地址必须为8字节边界地址 */
#endif
#endif
...
UINT8  FILE_DATA_BUF[ FILE_DATA_BUF_LEN ];	/* 外部RAM的文件数据缓冲区,缓冲区长度不小于一次读写的数据长度,起始地址必须为8字节边界地址 */
#else
UINT8  FILE_DATA_BUF[ FILE_DATA_BUF_LEN ];	/* 外部RAM的文件数据缓冲区,缓冲区长度不小于一次读写的数据长度,起始地址必须为8字节边界地址 */
#endif
...
void xDelayAfterWrite( void )			/* 写操作后延时 */
{
	...
	UINT32	count;
	for ( count = 5000; count != 0; count -- );  /* 延时200uS左右,2x20nS@50MHz */
// 以上代码会被优化掉，所以用嵌入式汇编
#endif
}
#endif
