...
#include <Philips\LPC21xx.H>						/* 目标板的专用初始化代码 */
...
/* 本例使用的LPC2114不开放系统总线,所以用I/O引脚模拟产生CH374的并口读写时序 */
...
void Write374Index( UINT8 mCmd )		/* 向索引端口写入索引地址 */
{
	...
	IO0SET |= ( (UINT32)mCmd << 8 ) | 0x00000010;  /* 向CH374的并口输出数据, 输出A0(P0.4)=1; */
	IO0DIR |= 0x0000FFF0;  /* 写操作所以数据输出, 设置CS,WR,RD,A0为输出 */
	IO0CLR |= 0x000000C0;  /* 输出有效写控制信号, 写CH374芯片的命令端口, A0(P0.4)=1; CS(P0.7)=0; WR=(P0.6)=0; RD(P0.5)=1; */
	IO0DIR = IO0DIR;  /* 该操作无意义,仅作延时,CH374要求读写脉冲宽度大于100nS */
	IO0SET |= 0x000000E0;  /* 输出无效的控制信号, 完成操作CH374芯片, A0(P0.4)=1; CS(P0.7)=1; WR=(P0.6)=1; RD(P0.5)=1; */
...
{
	IO0CLR |= 0x0000FF00;  /* 清并口输出 */
	IO0SET |= (UINT32)mData << 8;  /* 向CH374的并口输出数据 */
	IO0DIR |= 0x0000FF00;  /* 写操作所以数据输出 */
	IO0CLR |= 0x000000D0;  /* 输出有效写控制信号, 写CH374芯片的数据端口, A0(P0.4)=0; CS(P0.7)=0; WR=(P0.6)=0; RD(P0.5)=1; */
	IO0DIR = IO0DIR;  /* 该操作无意义,仅作延时,CH374要求读写脉冲宽度大于100nS */
	IO0SET |= 0x000000E0;  /* 输出无效的控制信号, 完成操作CH374芯片, A0(P0.4)=0; CS(P0.7)=1; WR=(P0.6)=1; RD(P0.5)=1; */
	IO0DIR &= 0xFFFF00FF;  /* 禁止数据输出 */
}
...
UINT8 Read374Data( void )			/* 从数据端口读出数据,索引地址自动加1 */
{
	...
	IO0CLR |= 0x000000B0;  /* 输出有效读控制信号, 读CH374芯片的数据端口, A0(P0.4)=0; CS(P0.7)=0; WR=(P0.6)=1; RD(P0.5)=0; */
	IO0DIR = IO0DIR;  /* 该操作无意义,仅作延时,CH374要求读写脉冲宽度大于100nS */
	mData = (UINT8)( IO0PIN >> 8 );  /* 从CH374的并口P0.15-P0.8输入数据 */
	IO0SET |= 0x000000E0;  /* 输出无效的控制信号, 完成操作CH374芯片, A0(P0.4)=0; CS(P0.7)=1; WR=(P0.6)=1; RD(P0.5)=1; */
	return( mData );
}
...
/* CH374FileEnum 与 CH374FileOpen 的唯一区别是当后者返回ERR_FOUND_NAME时那么对应于前者返回ERR_SUCCESS */
			...
			NewSize = i;  /* 新文件的长度 */
			SecCount = 1;  /* (NewSize+CH374vFileSize-1)/CH374vFileSize, 计算文件的扇区数,因为读写是以扇区为单位的 */
		}
		else {  /* 找到文件或者出错 */
			mStopIfError( i );
/*			printf( "Query\n" );
			...
			CH374vFileSize += CH374vFileSize - 1;  /* 默认情况下,以扇区方式读取数据时,无法读出文件尾部不足1个扇区的部分,所以必须临时加大文件长度以读取尾部零头 */
			i = CH374FileReadX( );  /* 从文件读取数据 */
			CH374vFileSize -= CH374vFileSize - 1;  /* 恢复原文件长度 */
			mStopIfError( i );
/*
		...
		mCmdParam.ReadX.mDataBuffer = &my_buffer[0];  指向文件数据缓冲区的起始地址
		CH374FileReadX();   直接读取从文件的第(CH374vFileSize*3)个字节开始的数据,前3个扇区被跳过

	    如果希望将新数据添加到原文件的尾部,可以移动文件指针
		i = CH374FileOpen( );
		mCmdParam.Locate.mSectorOffset = 0xffffffff;  移到文件的尾部,以扇区为单位,如果原文件是3字节,则从CH374vFileSize个字节处开始添加
		i = CH374FileLocate( );
...
/* 默认情况下,如果扇区数mCmdParam.WriteX.mSectorCount不为0那么CH374FileWriteX只负责写入数据而不修改文件长度,
   ...
   如果确定不会突然断电或者后面很快有数据不断写入则不必更新文件长度,可以提高速度并减少U盘损耗(U盘内部的内存寿命有限,不宜频繁改写) */
		...
		mStopIfError( i );
		printf( "Close\n" );
		mCmdParam.Close.mUpdateLen = 0;  /* 不要自动计算文件长度,如果自动计算,那么该长度总是CH374vFileSize的倍数 */
		i = CH374FileClose( );
		mStopIfError( i );
