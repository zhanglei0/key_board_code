...
*/
/* CH374的INT#引脚采用查询方式处理, 数据复制方式为"内部复制", 本程序适用于ATmega128单片机, 串口0输出监控信息,9600bps */
/* 本例以字节为单位读写U盘文件,读写速度较扇区模式慢,但是由于字节模式读写文件不需要文件数据缓冲区FILE_DATA_BUF,
   所以总共只需要600字节的RAM,适用于单片机硬件资源有限、数据量小并且读写速度要求不高的系统 */

/* ICCAVR -v -Wp -I\ICC\INCLUDE -Wf -Mavr_enhanced -Wl -L\ICC\LIB -m -bfunc_lit:0x8C.0x20000 -bdata:0x0100.0x0FFF -dram_end:0x0FFF -dhwstk_size:40 -ucrtatmega.o CH374HFB.C CH374HFB.A */
...
{  /* 为了提高速度,可以将该子程序做成宏以减少子程序调用层次 */
	...
	while ( ( SPSR & 0x80 ) == 0 );  /* 查询SPI状态寄存器以等待SPI字节传输完成 */
...
#define	Spi374Stop( )	{ PORTB |= 0x01; }  /* SPI结束 */
