...
void	CH374_READ_BLOCK( UINT8 mAddr, UINT8 mLen, PUINT8X mBuf )  /* 外部定义的被CH374程序库调用的子程序,从指定起始地址读出数据块 */
{
	Spi374Start( mAddr, CMD_SPI_374READ );
...
void	CH374_WRITE_BLOCK( UINT8 mAddr, UINT8 mLen, PUINT8X mBuf )  /* 外部定义的被CH374程序库调用的子程序,向指定起始地址写入数据块 */
{
	Spi374Start( mAddr, CMD_SPI_374WRITE );
...
PUINT8X	CH374_READ_BLOCK64( UINT8 mAddr, PUINT8X mBuf )  /* 外部定义的被CH374程序库调用的子程序,从双缓冲区读出64字节的数据块,返回当前地址 */
{
	UINT8	i;
	Spi374Start( mAddr, CMD_SPI_374READ );
...
PUINT8X	CH374_WRITE_BLOCK64( UINT8 mAddr, PUINT8X mBuf )  /* 外部定义的被CH374程序库调用的子程序,向双缓冲区写入64字节的数据块,返回当前地址 */
{
	UINT8	i;
	Spi374Start( mAddr, CMD_SPI_374WRITE );
